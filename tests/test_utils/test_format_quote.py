...
def test_missing_newline_separator():
    ...
    with pytest.raises(ValueError, match="Invalid quote format: missing newline separator"):
...
def test_multiple_newlines_in_author():
    ...
    with pytest.raises(ValueError, match="Invalid quote format: expected 'quote\\\\nauthor' format"):
...
def test_unicode_characters():
    ...
    assert format_quote(quote, words_per_line=2) == "Café naïve\nrésumé piñata\n- Unicode Author 中文"
