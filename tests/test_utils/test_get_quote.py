...
@patch(
    ...
    read_data=json.dumps([{"quote": "Test quote", "author": "Test Author"}]),
)
@patch("datetime.date")
...
@patch(
    "builtins.open",
    new_callable=mock_open,
    read_data=json.dumps([{"quote": "", "author": "Empty Quote"}]),
)
@patch("datetime.date")
...
@patch(
    "builtins.open",
    new_callable=mock_open,
    read_data=json.dumps([{"quote": "Quote without author key"}]),
)
@patch("datetime.date")
...
@patch("builtins.open", new_callable=mock_open, read_data=json.dumps(SAMPLE_QUOTES))
@patch("datetime.date")
def test_get_quote_various_ordinal_days(
    mock_date, mock_file, ordinal_day, expected_index
):
